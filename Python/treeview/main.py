...
class CheckboxTreeview(ttk.Treeview):
    ...
    def __init__(self, master=None, cols=None, **kw):
        ...
        # checked color
        self.tag_configure("checked", image=self.im_checked, background="#caebdb", foreground="#000000")
        # check / uncheck boxes on click
        self.bind("<Button-1>", self.box_click, True)
        # treeview events
        #self.bind('<<TreeviewSelect>>', print("Clicked1"))
        #self.bind('<<TreeviewOpen>>', print("Clicked2 "))
        #self.bind('<<TreeviewClose>>', print("Clicked3"))
        ...
        # Colors rows       
        self.tag_configure("odd", background="#FFF", foreground="#000")
        self.tag_configure("even", background="#EEE", foreground="#000")
    ...
    def box_click(self, event):        
        ...
        self.get_checked()
        # Show event
        # print(event, self.selection(), self.focus())
...
if __name__ == '__main__':
    ...
    # Heading
    style.configure("Treeview.Heading", font=("Roboto", 13), foreground="#252525", background="#fff")
    # Font, row height
    style.configure("Treeview", fieldbackground="#fff", font=("Roboto", 14), rowheight=33, highlightthickness=1, bd=1)
    # Selected row bg
    style.map('Treeview', background=[('selected', '#1f8c45')], foreground=[('selected', '#fff')])    
    
    t = CheckboxTreeview(root, show=["tree", "headings"], cols=("Age", "Name"))
    t.pack(expand=True, fill=tk.BOTH)
    
    # Items
    t.insert('', tk.END, id="0", values=("18", "Max"), tags=('odd',), open=False)
    t.insert('', tk.END, id="1", values=("23", "Alex"), tags=('even',), open=False)
    t.insert('', tk.END, id="2", values=("17", "Alice"), tags=('odd',), open=False)
    t.insert('', tk.END, id="3", values=("34", "John"), tags=('even',), open=False)
